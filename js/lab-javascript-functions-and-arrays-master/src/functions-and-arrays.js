...
function findLongestWord(words) {
  ...
  //  if (words[i].length > theLongestWord.length)theLongestWord = words[i];
  //}
  words.forEach((element) => {
    //element = words[i]
    if (element.length > theLongestWord.length) theLongestWord = element;
  });
...
function sum(numeros) {
  ...
  for (let i = 0; i < numeros.length; i++) {
    if (typeof numeros[i] === "string") result += numeros[i].length;
    if (typeof numeros[i] === "number") result += numeros[i];
    if (numeros[i] === true) result++;
    if (
      typeof numeros[i] != "string" &&
...
function averageNumbers(averageArray) {
  ...
  if (!averageArray.length) {
    return null;
  } else return result / averageArray.length;
}
